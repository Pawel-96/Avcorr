...
title='$s_'+str(J)+'$, example run'
contours_vals=[3,4,5]  #contours
...
ax.set_xlabel('$r_{\parallel}$' +' [Mpc/h]',fontsize=14)
ax.set_ylabel('$r_{\perp}$' +' [Mpc/h]',fontsize=14)
ax.set_title(title,fontsize=15,alpha=.7)


#making heatmap (logarithmic colorscale)
cf=plt.imshow(np.transpose(val[:,::-1]), cmap='cubehelix',interpolation = 'bicubic',aspect=1,vmin=np.amin(val),vmax=np.amax(val))
...
cb.set_label('$s_'+str(J)+'$', fontsize=13) #label
cb.ax.tick_params(labelsize=13) #ticks label size

plt.savefig('Heatmap_S_'+str(J)+'.png',bbox_inches='tight')