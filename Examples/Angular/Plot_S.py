...
ax.set_xlabel('$\theta[deg]$',fontsize=13)
ax.set_ylabel(r'$s_'+str(J)+'(\theta)$',fontsize=13)
#ax.set_yscale('log')
ax.set_xscale('log')
...
ax.fill_between(scale,value-error,value+error,color=color,alpha=ALPHA)
ax.plot(scale,value,color=color,linewidth=LW,label='result')
ax.legend()


plt.savefig('S_'+str(J)+'.png',bbox_inches='tight')